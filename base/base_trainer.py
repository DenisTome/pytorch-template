...
class BaseTrainer:
    ...
    def __init__(self, model, loss, metrics, optimizer, epochs, training_name,
        ...
        if self.with_cuda and (torch.cuda.device_count() > 1):
            if self.verbosity:
                self._logger.info("Let's use", torch.cuda.device_count(),
                                  "GPUs!")
            self.model = torch.nn.DataParallel(self.model)

        ensure_dir(os.path.join(save_dir, self.training_name))
        if resume:
    ...
    def _resume_checkpoint(self, resume_dir, epoch, iteration):
        ...
        if not model_name:
            self._logger.error(
                'Model {}/ckpt_eph{:02d}_iter{:06d} does not exist'.format(
                    resume_dir, epoch, iteration))
        # load model
        resume_path = os.path.join(resume_dir, model_name[0])
